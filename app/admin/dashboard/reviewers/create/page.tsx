...
export default function CreateReviewerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PROSES 1: Upload ke Supabase Storage Bucket 'reviewer-avatars'
      if (avatarFile) {
      ...
      // PROSES 2: Simpan Data ke Tabel 'reviewers'
      ...
      setMessage({ type: 'success', text: 'Reviewer successfully injected into the expert panel pipeline.' })
      setName('')
      setRoleTitle('')
      setShortBio('')
      setAvatarFile(null)
    ...
    <main className="max-w-4xl w-full mx-auto px-6 py-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="space-y-1">
          <Link 
            href="/admin/dashboard/reviewers" 
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-white flex items-center gap-3">
            <UserPlus className="w-8 h-8 text-purple-500" /> Deploy New Reviewer
          </h1>
          <p className="text-sm text-slate-400">Establish credentialed board peers for advanced truth verification tags.</p>
        </div>
        <div className="hidden md:block">
          <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-2xl flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-slate-300 font-mono">Review Node Ready</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col items-center text-center">
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-6">Expert Avatar</label>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Expert Full Name</label>
                <input 
                  type="text" 
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="e.g. Dr. Latoya Irby, CFA"
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Financial Review Board Chair"
                  className="w-full bg-slate-950 border border-slate-800 focus:border-purple-500/50 text-slate-200 text-sm px-5 py-4 rounded-2xl transition-all outline-none"
                  required
                />
              </div>
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Reviewer Core Statement / Bio</label>
              <textarea 
                rows={5}
                value={shortBio}
                onChange={(e) => setShortBio(e.target.value)}
                placeholder="State the reviewer's professional authority, verifying their status to audit content accuracy..."
            ...
            <div className="pt-4">
              ...
              >
                {loading ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" /> Committing Expert Node...
                  </>
                ) : (
                  'Deploy Reviewer Node'
                )}
              </button>
            </div>
